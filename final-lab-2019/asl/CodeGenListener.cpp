...
void CodeGenListener::exitAssignStmt(AslParser::AssignStmtContext *ctx) {
	...
	string          temp2 = getAddrDecor(ctx->expr());
	...
	// ni arrays ni construct d'arrays
	else {
...
void CodeGenListener::enterIfStmt(AslParser::IfStmtContext *ctx) { DEBUG_ENTER(); }
...
void CodeGenListener::enterProcCall(AslParser::ProcCallContext *ctx) { DEBUG_ENTER(); }
...
void CodeGenListener::enterMaximExpr(AslParser::MaximExprContext *ctx) {}
void CodeGenListener::exitMaximExpr(AslParser::MaximExprContext *ctx) {
	putCodeDecor(ctx, getCodeDecor(ctx->maxim()));
	putAddrDecor(ctx, getAddrDecor(ctx->maxim()));
	putOffsetDecor(ctx, "");
}

void CodeGenListener::enterMaxim(AslParser::MaximContext *ctx) {}
void CodeGenListener::exitMaxim(AslParser::MaximContext *ctx) {
	...
	std::string maxx = "%"+codeCounters.newTEMP();
	std::string temp1 = "%"+codeCounters.newTEMP();
	std::string temp2 = "%"+codeCounters.newTEMP();
	codeaux = codeaux || instruction::ILOAD(index, "0");
	codeaux = codeaux || instruction::ILOAD(temp1, std::to_string(s)); // mida del tipus
	if (not Symbols.isParameterClass(addr)) {
		TypesMgr::TypeId tc = Types.getArrayElemType(t1);

		codeaux = codeaux || instruction::LOADX(temp2, addr,index); // temp2 = addr[index = 0]
		if (Types.isIntegerTy(tc)) codeaux = codeaux || instruction::ILOAD(maxx, temp2); // maxx = a[0]
		else {
			std::string tp2 = "%"+codeCounters.newTEMP();
			codeaux = codeaux || instruction::FLOAT(tp2, temp2);
			codeaux = codeaux || instruction::FLOAD(maxx, tp2); // cas ser float
		}
		codeaux = codeaux || instruction::ADD(index, index, temp1); // index += mida tipus
		for(int i = 1; i < size; i++){
			std::string      label = codeCounters.newLabelIF();
			std::string labelEndIf = "false"+label+to_string(i);
			codeaux = codeaux || instruction::LOADX(temp2, addr, index); //
			codeaux = codeaux || instruction::ADD(index, index, temp1);
			std::string tmp = "%"+codeCounters.newTEMP();
			if (Types.isIntegerTy(tc)) codeaux = codeaux || instruction::LT(tmp, maxx, temp2);
			else codeaux = codeaux || instruction::FLT(tmp, maxx, temp2);
			codeaux = codeaux || instruction::FJUMP(tmp, labelEndIf);
			if (Types.isIntegerTy(tc)) codeaux = codeaux || instruction::ILOAD(maxx, temp2);
			else {
				std::string tp2 = "%"+codeCounters.newTEMP();
				codeaux = codeaux || instruction::FLOAD(tp2, temp2);
				codeaux = codeaux || instruction::FLOAT(maxx, tp2);
			}
			codeaux = codeaux || instruction::LABEL(labelEndIf);
		}
		code = code || codeaux;
		putAddrDecor(ctx, maxx);
		putCodeDecor(ctx, code);
	}
	else {
		TypesMgr::TypeId tc = Types.getArrayElemType(t1);
		std::string tmp = "%"+codeCounters.newTEMP();
		codeaux = codeaux || instruction::LOAD(tmp, addr);
		codeaux = codeaux || instruction::LOADX(temp2, tmp,index);
		if (Types.isIntegerTy(tc)) codeaux = codeaux || instruction::ILOAD(maxx, temp2);
		else {
			std::string tp2 = "%"+codeCounters.newTEMP();
				codeaux = codeaux || instruction::FLOAT(tp2, temp2);
				codeaux = codeaux || instruction::FLOAD(maxx, tp2);
		}
		codeaux = codeaux || instruction::ADD(index, index, temp1);
		...
		for(int i = 1; i < size; i++){
			...
			if (Types.isIntegerTy(tc)) codeaux = codeaux || instruction::LT(tmp2, maxx, temp2);
			else codeaux = codeaux || instruction::FLT(tmp2, maxx, temp2);
			codeaux = codeaux || instruction::FJUMP(tmp2, labelEndIf);
			if (Types.isIntegerTy(tc)) codeaux = codeaux || instruction::ILOAD(maxx, temp2);
			else {
				std::string tp2 = "%"+codeCounters.newTEMP();
				codeaux = codeaux || instruction::FLOAD(tp2, temp2);
