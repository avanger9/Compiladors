...
int main(int argc, const char *argv[])
{
    ...
    // Create a Listener that looks for variables and function
    // declarations in the tree
    // and stores required information
    SymbolsListener symboldecl(types, symbols, decorations, errors);
    // Traverse the tree using this listener, to collect information about 
    // 
    // declared identifiers
    walker.walk(&symboldecl, tree);

    // Create another Listener that will perform type checkings wherever
    // it is needed
    ...
    code mycode;
    // Create a third listener that will generate code for each part of
    // the tree
    CodeGenListener codegenerator(types, symbols, decorations, mycode);
    // Traverse the tree using this listener, so code is generated and
    // stored in 'mycode'
