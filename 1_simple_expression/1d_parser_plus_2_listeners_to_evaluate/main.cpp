...
public:
  ...
  void exitE(ExprParser::EContext *ctx) {  // e : e MULT e | e ADD e | INT ;
    if (ctx->INT())  // if this EContext has a child INT
    // Alternative:
    // if (ctx->children.size() == 1) { // if this EContext has one child
      stack.push(std::stoi(ctx->INT()->getText()));
    else {
      if (ctx->PL() or ctx->PR()) return;
      int right = stack.top();
      stack.pop();
      int left = stack.top();
      stack.pop();
      if (ctx->MULT())
      // Alternative, with labels to the operators (e op=MULT e):
