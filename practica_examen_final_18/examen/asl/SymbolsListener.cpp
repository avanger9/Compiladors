...
void SymbolsListener::exitTypeR(AslParser::TypeRContext *ctx) {
	if (ctx->type()) 
		...
		putTypeDecor(ctx, getTypeDecor(ctx->pair()));
}
...
void SymbolsListener::enterPair(AslParser::PairContext *ctx) {}
void SymbolsListener::exitPair(AslParser::PairContext *ctx) {
	TypesMgr::TypeId t1,t2;
	t1 = getTypeDecor(ctx->type(0));
	t2 = getTypeDecor(ctx->type(1));
	putTypeDecor(ctx, Types.createPairTy(t1, t2));
}
